document.addEventListener('DOMContentLoaded', async function() {
    ...
    const x = variables.map(c => {
        if (['high_blood_pressure', 'anaemia', 'smoking', 'diabetes', 'sex'].includes(c)) {
            return d3.scaleOrdinal().domain(["0", "1"]).range([padding / 2, size - padding / 2]);
        } else {
            return d3.scaleLinear().domain(d3.extent(data, d => +d[c])).rangeRound([padding / 2, size - padding / 2]);
        }
    });
    ...
    legend.append("text").attr("x", 10).attr("y", 25).text("Non-Death Event").style("font-size", "12px").attr("alignment-baseline", "middle");
    ...
    cell.append("rect").attr("fill", "none").attr("stroke", "#aaa").attr("x", padding / 2 + 0.5).attr("y", padding / 2 + 0.5).attr("width", size - padding).attr("height", size - padding);
    ...
    // Checkbox for death event filtering
